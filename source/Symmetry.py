...
def adaptive_binary_image(im, white_background, min_feature_size, std_dev, mask):
    ...
    binary = threshold_adaptive(image,block_size=local_size+1)
...
im = skimio.imread('/users/jenyu/documents/matlab/C9 PbS_5.tif',as_grey=True,plugin='matplotlib')
...
for e in metric_list_final:
    if e> 0.5:
        count_4 +=1
    if e<-0.5:
        count_6 +=1
ratio = round(count_4/(count_4+count_6),3)
print(count_4,count_6, ratio)
print(len(pts), len(vor.regions))
