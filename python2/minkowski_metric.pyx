...
cpdef minkowski(np.ndarray[DUBTYPE_t, ndim=2] vor_vertices, vor_regions, int l, limits):
    ...
    cdef int region_index, region_vert_index, vert_count, facet_index
    cdef int m
        ...
        if vert_count <= 20:
            ...
            if vert_count and np.all(np.not_equal(region[:vert_count],-1)):
                ...
                for region_vert_index in range(vert_count):
                    ...
                    x1,y1 = vor_vertices[<unsigned int>region[region_vert_index]]
                    x2,y2 = vor_vertices[<unsigned int>region[<unsigned int>((region_vert_index+1) % vert_count)]]
                    x3,y3 = vor_vertices[<unsigned int>region[<unsigned int>((region_vert_index+2) % vert_count)]]
